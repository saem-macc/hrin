class PostsController < ApplicationController
  ...
  before_action lambda {
    resize_before_save(post_params[:image])
  }, only: [ :create, :update ]
  ...
  private
    ...
    def post_params
      params.expect(post: [ :title, :image ])
    end
    ...
    def resize_before_save(image_param)
      ...
      begin
        img = ImageProcessing::Vips.source(image_param)
        .resize_to_fill(800, 800)
        .call(save: false)
        img.write_to_file(image_param.tempfile.path, Q: 35, strip: true)
